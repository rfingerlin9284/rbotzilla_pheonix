...
start)
    ...
    RBOT_MIN_SIGNAL_CONFIDENCE=0.72 RBOT_BE_TRIGGER_R=0.85 RBOT_SCALE_OUT_TRIGGER_R=1.10 RBOT_TRAIL_TRIGGER_R=1.50 RBOT_TRAIL_DISTANCE_R=0.35 RBOT_PROFIT_EXTRACTION_MODE=1 RBOT_GREEN_LOCK_PIPS=10.0 RBOT_GREEN_LOCK_MIN_PROFIT_PIPS=10.0 RBOT_MAX_LOSS_USD_PER_TRADE=150 RBOT_MAX_LOSS_USD_EXTENDED_PER_TRADE=180 RBOT_ALLOW_EXTENDED_DOLLAR_STOP=1 RBOT_MAX_PAIRS_PER_PLATFORM=12 RBOT_MAX_ACTIVE_POSITIONS=12 RBOT_MAX_NEW_TRADES_PER_CYCLE=6 RBOT_TP_COOLDOWN_MINUTES=3 RBOT_AUTO_RESIZE_POSITIONS=1 nohup "$PYTHON" headless_runtime.py --broker all \
        >> "$LOG" 2>&1 &
    echo $! > "$PIDFILE"
    sleep 2
...
# ── watchdog ──────────────────────────────────────────────────────────────────
watchdog)
    hdr "ORDER WATCHDOG — background monitor"
    require_venv
    info "Starting order watchdog (Ctrl+C to stop)..."
    info "Alerts → logs/watchdog_alerts.log"
