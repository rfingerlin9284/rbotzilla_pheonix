...
class CoinbaseConnector:
    ...
    def place_oco_order(self, product_id: str, entry_price: float, stop_loss: float, 
        ...
        NOTE: Coinbase doesn't have native OCO, so this creates linked orders to enforce OCO behavior
        ...
        try:
            ...
            # Validate API credentials for runtime environments
            if self.environment in ("live", "sandbox"):
                if (
                    not self.api_key
                    or self.api_key in ("your_live_api_key", "your_sandbox_api_key")
                    or not self.api_secret
                    or self.api_secret in ("your_live_secret", "your_sandbox_secret")
                ):
                    self.logger.error(f"{self.environment.upper()} Coinbase API credentials not configured")
                    return {
                        "success": False,
                        "error": f"{self.environment.upper()} API credentials not configured",
                        "latency_ms": 0,
                ...
                if not entry_response["success"]:
                    return {
                        "success": False,
                        "error": f"{self.environment.upper()} entry order failed: {entry_response['error']}",
                ...
                if avg_latency > self.max_placement_latency_ms:
                    self.logger.error(f"Coinbase OCO avg latency {avg_latency:.1f}ms exceeds Charter limit - CANCELLING ORDERS")
                ...
                if all_successful:
                    self.logger.info(
                        f"Coinbase OCO placed: {product_id} | Entry: {entry_price} | "
                    ...
                    return {
                        "success": False,
                        "error": "OCO placement failed - some orders unsuccessful",
                        "latency_ms": avg_latency,
                        "execution_time_ms": execution_time,
                        "broker": "Coinbase",
                        "environment": self.environment,
                ...
                return {
                    "success": False,
                    "error": f"Unsupported Coinbase environment: {self.environment}",
                    "latency_ms": 0,
                    "execution_time_ms": (time.time() - start_time) * 1000,
                    "broker": "Coinbase",
                    "environment": self.environment
...
if __name__ == "__main__":
    ...
    try:
        ...
        if oco_result["success"]:
            print(f"✅ OCO order flow completed successfully")
            print(f"   Entry Order ID: {oco_result.get('entry_order_id', 'N/A')}")
            print(f"   Avg Latency: {oco_result['latency_ms']:.1f}ms")
        else:
            print(f"❌ OCO order flow failed: {oco_result.get('error', 'Unknown error')}")
        ...
        print("✅ OCO linked-order logic implemented")
