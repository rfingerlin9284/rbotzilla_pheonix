...
class SwarmBot:
    ...
    def __init__(self, position: Position, pin: int = None, broker_connector=None):
        ...
        if broker_connector is None:
            raise RuntimeError("Broker connector is required for SwarmBot in PAPER/LIVE runtime")
    ...
    def _calculate_current_price(self) -> float:
        ...
        try:
            ...
            raise RuntimeError(f"No broker connector price feed available for {self.position.symbol}")
    ...
    def _calculate_volatility_trail_distance(self, current_price: float) -> float:
        ...
        Uses ATR-style approximation
        """
        # ATR (Average True Range) approximation
        # In production, this would use real price data
        base_atr = current_price * 0.008  # 0.8% of current price as ATR proxy
...
class SwarmManager:
    ...
    def __init__(self, pin: int = None, broker_connector=None):
        ...
        if broker_connector is None:
            raise RuntimeError("Broker connector is required for SwarmManager in PAPER/LIVE runtime")
...
if __name__ == "__main__":
    # Self-test with sample positions
