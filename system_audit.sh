...
SCAN_DIRS=(
  "$WORK_DIR"
  "$WORK_DIR/_source_repos/R_H_UNI"
  "$WORK_DIR/_source_repos/rick_clean_live"
  "$WORK_DIR/_source_repos/RICK_LIVE_CLEAN_FROZEN"
)
...
test_crash_handling() {
  ...
  if pgrep -f "oanda_trading_engine.py" > /dev/null 2>&1; then
    log_info "✅ OANDA Headless Engine: RUNNING"
    mem=$(ps aux | grep "oanda_trading_engine.py" | grep -v grep | awk '{print $6}' | head -1)
    log_info "  Memory usage: ${mem}KB"
  else
    log_warn "⚠️  OANDA Headless Engine: NOT RUNNING"
  fi
